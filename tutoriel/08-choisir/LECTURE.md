...
Nos programmes tournent, mais leurs résultats restent inégaux : une recherche manque une formulation, une réponse invente une durée et une adaptation dégrade l’ancien format. 🙂 Nous avons aussi séparé la demande d’outil de son exécution, construit un serveur MCP et écrit une procédure réutilisable. Nous avons donc assez d’éléments pour décider ce que nous avons envie de leur confier.
...
Retrouvons notre équipe fictive de suivi de prix. Elle hésite entre automatiser une vérification de catalogue, préparer des tests et analyser des incidents clients. Ces demandes paraissent proches ; les données, les conséquences et les solutions diffèrent pourtant beaucoup.
...
Prenez la fiche du modèle utilisé dans la partie 3, SmolLM2-360M-Instruct. Nous y trouvons des informations sur la famille de modèles, des données et des évaluations. C’est un point de départ. La fiche ne donne pas le nom de chaque personne ayant contribué à chaque texte.[^p8-carte]
...
Nous avons commencé par ouvrir la boîte noire. Au fil des huit parties, nous avons retracé l’histoire de l’IA, construit un petit modèle, lancé un modèle local, développé avec un assistant, observé la boucle d’outils avec un banc, construit un serveur MCP, écrit un skill, puis assemblé notre propre assistant documentaire. Enfin, nous avons regardé ce que tous ces outils demandent : du travail humain, des données, des ressources et du temps pour vérifier leurs productions.

Selon moi, leur intérêt tient justement à la possibilité de les adapter à nos besoins. Générer beaucoup de code ne suffit pas à justifier que nous réorganisions le métier autour d’un agent. Si l’outil nous éloigne de ce que nous livrons au point de ne plus pouvoir l’expliquer, il a raté sa place.
