...
async def consulter(operation, valeur=None):
    params = StdioServerParameters(command=sys.executable, args=[str(ROOT/'serveur.py')], cwd=ROOT)
    async with Client(params, read_timeout_seconds=10) as client:
        ...
        return {'operation':operation, 'valeur':valeur,
...
def main():
    ...
    resultat=asyncio.run(consulter(args.operation,args.valeur))
